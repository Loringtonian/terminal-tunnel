...
| ⌥ Esc           | close the picker, when one is open                      |
| ⌥ \             | next wallpaper                                          |
| ⌥ C             | centre the front window                                 |
| ⌥ Esc           | exit, putting every window it moved back where it was   |

Cycling centres the chosen window and cuts the hole around it. Launching Focus Mode, or Cmd+Tabbing to it, hands keyboard focus straight to the framed window, so you can type without clicking. Clicking into the framed window keeps the art up.

The two pools remember where you were independently. If you are on a terminal and press ⌥ ↑, you go to the editor you last used; pressing ⌥ ← then takes you back to the same terminal, not the one before it. Only a second press in the same direction steps along a pool, so alternating between a terminal and an editor bounces between exactly those two windows.

While the picker is open it takes the keyboard: a bare number key picks that row and Esc closes it, no modifier needed. Switching to any other app (a browser, Finder) drops the art and restores the windows; coming back brings it up again on the window you were last on. New windows are picked up on your next cycle and appear at the end of the order.
...
python3 focus_mode.py --setup
python3 focus_mode.py ~/Pictures/some_wallpaper.png
```

`--setup` lists the apps you have open and asks which are your terminals and which are your editors, then writes them to the config file. Everyone's editor is different, so run it once before anything else — the built-in defaults are only a guess at common apps.
...
- `terminals` and `editors` are app names as macOS reports them, set by `--setup` or edited here by hand. Defaults, used when the file has no such key: Terminal, iTerm2, Warp, Ghostty, Alacritty, kitty and TextEdit, CotEditor, BBEdit, Sublime Text, Code, Cursor, Zed. `python3 focus_mode.py --list` prints the windows currently in each pool, which is the quickest way to check a name.
...
```
python3 focus_mode.py --setup     # choose which apps count as terminals and editors
